	...
	<div>
		<view>
			...
			<uni-popup ref="inputDialog" type="dialog">
				<uni-popup-dialog title="密码修改" style="display: flex">
					<uni-section title="原密码" type="line" padding>
						<uni-easyinput errorMessage v-model="value" focus placeholder="请输入原密码"
							@input="input"></uni-easyinput>
					</uni-section>
					<uni-section title="新密码" type="line" padding>
						<uni-easyinput errorMessage v-model="value" focus placeholder="请输入新密码"
							@input="input"></uni-easyinput>
						<uni-easyinput errorMessage v-model="value" focus placeholder="请重新输入新密码"
							@input="input"></uni-easyinput>
					</uni-section>
				</uni-popup-dialog>
			</uni-popup>
		...
		<view class="example">
			...
			<uni-card v-if="nickname !== '点击登录'">
				<view class="countshow">
					<view class="article_count"> 总发表：{{ total }} </view>
					<view class="article_count"> 总发言：{{ total }} </view>
				</view>
			</uni-card>
			<uni-card v-if="nickname !== '点击登录'">
				<ul class="menu2">
					...
					<li class="custom-li" @click="inputDialogToggle">修改密码</li>
...
<script setup>
	...
	onMounted(() => {
		console.log("挂载监听onMounted");
		uni.$on("loginSuccess", checklogin);
		uni.$on("userinfoSuccess", updateInfo);
	});

	onUnmounted(() => {
		console.log("挂载监听onUnmounted");
		uni.$off("loginSuccess", checklogin);
		uni.$off("userinfoSuccess", updateInfo);
	});
	...
	const checklogin = () => {
		readToken()
			...
			.catch(() => {
				...
				console.log("用户未登录或登录态已过期，引导用户进行登录操作")
				uni.navigateTo({
					...
					fail(err) {
						console.log('跳转/pages/component/login/login失败', err);
					}
